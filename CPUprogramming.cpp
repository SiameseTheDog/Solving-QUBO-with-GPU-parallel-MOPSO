...
std::vector<float> multiplyQxBLAS(const std::vector<std::vector<float>>& Q, const std::vector<float>& x) {
    std::vector<float> Y(Q.size(), 0.0f);
    cblas_sgemv(CblasRowMajor, CblasNoTrans, Q.size(), Q.size(), 1.0f,
                &Q[0][0], Q.size(), &x[0], 1, 0.0f, &Y[0], 1);
    return Y;
}
// // Overload for std::vector<int>
// std::vector<float> multiplyQxBLAS(const std::vector<std::vector<float>>& Q, const std::vector<int>& x) {
//     std::vector<float> Y(Q.size(), 0.0f);
//     cblas_sgemv(CblasRowMajor, CblasNoTrans, Q.size(), Q.size(), 1.0f,
//                 &Q[0][0], Q.size(), &x[0], 1, 0.0f, &Y[0], 1);
//     return Y;
...
int main() 
{
    ...
    // Declare particles and global best
...
/////////////////////////////// Multiple Threaded //////////////////////////////
    ...
    for (int t = 1; t <= t_max; t++) 
    {
        for (auto& p : particles) 
        {
            ...
            // Calculate objective function values
            std::vector<float> Y = multiplyQxBLAS(Q, p.position);
            std::vector<float> objectives = {f1(Y, p.position), f2(p.position)};
    ...
    std::cout << "Single-threaded execution time: " << elapsedSingleThreaded.count() << " seconds\n";
    std::cout << "Multi-threaded execution time: " << elapsedMultiThreaded.count() << " seconds\n";
