        ...
        int numParticles) 
{
    ...
    if (idx < numParticles) {
        ...
        if ((f1 <= personalBestf1 && f2 < personalBestf2) || (f1 < personalBestf1 && f2 <= personalBestf2))
        {
...
int main(int argc, char* argv[]) 
{
    ...
    const int numParticles = 32768; // Number of particles swarm, may vary
    ...
    std::vector<float> h_particlesPositions (numParticles *Q_size, 0.0f);
    std::vector<float> h_particlesVelocities (numParticles *Q_size, 0.0f);
    std::vector<float> h_personalBestValues (numParticles *2, 0.0f);
    std::vector<float> h_personalBestPositions (numParticles *Q_size, 0.0f);
    std::vector<float> h_globalBestPositions (numParticles *Q_size, 0.0f);
    std::vector<float> h_globalBestValues (numParticles *2, 0.0f);
    ...
    // cudaMemcpy(d_particlesPositions, h_particlesPositions.data(), numParticles * Q_size * sizeof(float), cudaMemcpyHostToDevice);
    ...
    // cudaMemcpy(h_particlesPositions.data(), d_particlesPositions, numParticles * Q_size * sizeof(float), cudaMemcpyDeviceToHost);

    // std::cout << "positions:" << std::endl;
    // for (int i = numParticles - 16; i < numParticles; ++i) {
    //     for (int j = 0; j < Q_size; ++j) {
    ...
    float* d_Y;
    float* d_Q;
    // float* h_Y = new float[numParticles * Q_size];;
    ...
    for (int t = 1; t <= t_max; ++t) 
    {
        ...
        cudaMemcpy(h_personalBestValues.data(), d_personalBestValues, numParticles * 2 * sizeof(float), cudaMemcpyDeviceToHost);
        cudaMemcpy(h_personalBestPositions.data(), d_personalBestPositions, numParticles * Q_size * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    std::vector<float> h_YFinal (archiveFinalSize *Q_size, 0.0f);
    float* d_YFinal;
    ...
    cudaMemcpy(h_YFinal.data(), d_YFinal, Q_size * archiveFinalSize * sizeof(float), cudaMemcpyDeviceToHost);
    ...
    float minf1 = std::numeric_limits<float>::max();
    ...
    std::cout << "GPU execution time for " << numParticles <<" particles: " << elapsedTime.count() << " seconds\n";
    // std::cout << "Single-threaded execution time for " << numParticles <<" particles: " << elapsedMultiThreaded.count() << " seconds\n";
    // Output the best rounded solution and its f1 value
    std::cout << "Best solution found for" << filePath << ": "<< " " << minf1 << std::endl << std::endl;
